...
// -> 某设备 -> port 设备上的某个服务(进程)
// 一台设备上可以很多端口使用,有多个http服务 多个网站 
// 不要使用一些特殊端口  
const http = require('http');
const fs = require('fs'); // file system 文件系统模块
...
const server=http.createServer((req,res) =>{
    ...
    if(req.method === 'GET' && 
        ...
        fs.readFile(
            path.join(__dirname,'public','index.html'),// 路径拼接 
            //  异步 callback 回调函数
            (err,content)=> {
            // 后端稳定为主 前端体验为主 
            if(err){
    ...
    // 协议 http:// localhost 域名 端口 /style.css path querystring 路径和查询字符串
    if(req.method ==='GET' && req.url =='/style.css'){
        fs.readFile(
        path.join(__dirname,'public','style.css'),
        (err,content) => {
            if(err){
                res.writeHead(500);// 服务器端错误
                res.end('Server error')
                return ;
            }
